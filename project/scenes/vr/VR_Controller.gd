...
onready var help_text := get_node("Controller_GUI")
...
func update_camera_position_on_warp():
	...
	var camera_offset = -(get_parent().get_node("StarSimVRCamera").global_transform.origin.y - get_parent().global_transform.origin.y)
	get_parent().global_transform.origin = origin + Vector3(0,camera_offset,1)
